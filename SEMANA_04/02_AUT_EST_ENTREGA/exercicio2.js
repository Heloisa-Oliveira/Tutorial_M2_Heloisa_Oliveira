...
// Retorna todos registros da tabela de Formação (é o R do CRUD - Read)
...
// Retorna todos registros da parte da tabela de Experiência (é o R do CRUD - Read)
app.get('/listaexperiencia', (req, res) => {
    res.statusCode = 200;
    res.setHeader('Access-Control-Allow-Origin', '*');
    var db = new sqlite3.Database(DBPATH);
    var sql = 'SELECT * FROM experiencia ORDER BY empresa';
        db.all(sql, [], (err, rows) => {
            if(err) {
                throw err;
            }
...
// Retorna todos registros da parte da tabela de Habilidades (é o R do CRUD - Read)
...
// Retorna todos registros da parte da tabela de Personalidade (é o R do CRUD - Read)
...
// Retorna todos registros da parte da tabela de Realizações (é o R do CRUD - Read)
app.get('/listarealizacoes', (req, res) => {
    res.statusCode = 200;
    res.setHeader('Access-Control-Allow-Origin', '*');
    var db = new sqlite3.Database(DBPATH);
    var sql = 'SELECT * FROM realizacoes ORDER BY titulo';
        db.all(sql, [], (err, rows) => {
            if(err) {
                throw err;
            }
...
// Retorna todos registros da parte da tabela de Pessoas (é o R do CRUD - Read)
app.get('/listapessoas', (req, res) => {
    res.statusCode = 200;
    res.setHeader('Access-Control-Allow-Origin', '*');
    var db = new sqlite3.Database(DBPATH);
    var sql = 'SELECT * FROM pessoas ORDER BY telefone';
        db.all(sql, [], (err, rows) => {
            if(err) {
                throw err;
            }
